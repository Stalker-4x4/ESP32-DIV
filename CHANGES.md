...
> Примечание: `radio.begin()` на общей с SD SPI-шине может не поднимать несущую до `SPI.end()` +
> `SPI.begin(пины nRF24)` — если сам джамминг не работает, это отдельный вопрос переключения SPI,
> не связанный с индикацией.
