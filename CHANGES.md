...
Версия прошивки на экране увеличивается при каждой перепрошивке: `v1.7.0-1` … текущая **`v1.7.0-12`**
...
**Proto Kill / BLE Jammer реально не излучали (v1.7.0-12).** Причина та же ранний выход
`SPIClass::begin()`: глобальный SPI оставался на пинах SD, `SPI.begin(пины nRF24)` их не переключал,
поэтому `radio.begin()` общался по пинам SD и несущая не выходила. Исправление: в `prokillSetup()` и
`blejamSetup()` перед инициализацией радио теперь `SPI.end(); SPI.begin(13, 11, 12, 4);` —
`end()` обнуляет `_spi`, так что следующий `begin()` действительно ставит пины nRF24 (внутренний
`SPI.begin()` из RF24 после этого no-op и сохраняет их). В `prokillExit()`/`BleJammer::exit()`
добавлено `SPI.end(); restoreSdAfterSharedSpi();` — возврат шины под SD и перемонтирование карты.
